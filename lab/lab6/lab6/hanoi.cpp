...
// This is the function that you will be writing
void play_automatic(int num_disks, int start_tower, int end_tower);
void play_automatic_h(int num_disks, int start_tower, int end_tower, int mid_tower);

// These are the functions that you may call.
...
// These are functions that you may NOT call directly
// (because they won't help you, not because I'm mean)
...
int main() 
{
	...
	if(game_style == 'm')
	{
		...
		do{
			...
			game_over = check_for_win(end_tower);
		}while(!game_over);
	...
	// If the user wants to run their automatic solution
		...
		play_automatic(height, start_tower, end_tower);

		if(check_for_win(end_tower))
		{
			cout << "You have completed the Tower of Hanoi!!!\n";
		}else{
...
void play_automatic(int num_disks, int start_tower, int end_tower)
{
  play_automatic_h(num_disks, start_tower, end_tower, third_tower(start_tower, end_tower));
}//end play_automatic

//void play_automatic_h(int num_disks, int start_tower, 
//int end_tower, int mid_tower)
//Purpose: help the play_automatic function
//Parameters: int num_disks, int start_tower, int end_tower, int mid_tower
//Returns: void
void play_automatic_h(int num_disks, int start_tower, int end_tower, int mid_tower)
{
  ...
  if (num_disks == 1)
    {
      move_disk(start_tower, end_tower);
      return;
    }//end if

  else{
    play_automatic_h(num_disks - 1, start_tower, mid_tower, end_tower);
    move_disk(start_tower, end_tower);
    play_automatic_h(num_disks - 1, mid_tower, end_tower, start_tower);
  }//end else
}//end play_automatic_h
...
bool move_disk(int from_tower, int to_tower)
{
	...
	int move_index;
	int move_disk = move_helper(from_tower,  move_index);
	int onto_index;
	int onto_disk = move_helper(to_tower, onto_index);
